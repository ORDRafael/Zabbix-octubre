...
if (CWebUser::getType() == USER_TYPE_ZABBIX_USER) {
	...
	if (isset($_GET['filter_hostids'])) {
	$hostUrl = $_GET['filter_hostids']; // Esto ahora capturará el arreglo.
	
	// Verifica si hay múltiples valores
	if (is_array($hostUrl)) {
		foreach ($hostUrl as $hostId) {
			echo "ID del host capturado desde la URL: $hostId<br>";

			// Verifica si cada ID está en el array de hostids
			if (in_array($hostId, $hostids)) {
				echo "El hostid $hostId es válido.<br>";
			} else {
		...
		// Verifica si el ID está en el array de hostids
		if (in_array($hostUrl, $hostids)) {
			echo "El hostid $hostUrl es válido.";
		} else {
			header("location: error.php");
			exit; // Asegúrate de salir después de redirigir
		}
	...
	echo "No se encontró el parámetro 'filter_hostids' en la URL."; 
}
